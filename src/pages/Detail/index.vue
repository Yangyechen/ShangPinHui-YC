...
export default {
  ...
  methods: {
    // 售卖属性高亮
    changeActive(salAttrValue, arr) {
      // console.log(a,b)
      // 遍历售卖属性值的isChecked全部设为0 不选中
      arr.forEach(item => {
        item.isChecked = 0
      })
      // 点击的那个属性值应该为1
      salAttrValue.isChecked = 1
    },
    // 表单元素修改产品个数
    changeSkuNum(event) {
    ...
    async addShopCart() {
      ...
      try {
        ...
        // 不建议
        // this.$router.push({name: 'addcartsuccess', query: {skuInfo: this.skuInfo, skuName: this.skuNum}})
        // 产品信息数据比较复杂 通常使用会话存储 一些简单的数据 通过query参数传递给服务器
        // 本地存储||会话存储 存储的是字符串
